 ...
 * @author Varsha
 */
@WebServlet(name = "AddModelServ", urlPatterns = {"/AddModelServ"})
public class AddModelServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                AddModel cm = new AddModel();
                AddModel cm1 = new AddModel();

                cm.setModelName(Model);

                ModelDao cd = new ModelDao();
                boolean b = cd.saveAddModel(cm);
                if (b) {
                    int i = 1;
                    HttpSession session = request.getSession();
                    session.setAttribute("i", i);
                    session.setAttribute("msg", "Model No Added Sucessfully !!!");
                    response.sendRedirect("Admin/admin/AddModelNo.jsp");

                } else {
                    cm1 = cd.getAddModel(cm);
                    if (!cm1.isDeletestatus()) {

                        cm1.setDeletestatus(true);
                        boolean b1 = cd.updateAddModelb(cm1);
                        if (b1) {
                            int i = 1;
                            HttpSession session = request.getSession();
                            session.setAttribute("i", i);
                        ...
                        int i = 2;
                            HttpSession session = request.getSession();
                            session.setAttribute("i", i);
                            session.setAttribute("msg", "Model No  Allreday Added!!!");
