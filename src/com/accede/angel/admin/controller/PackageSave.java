 ...
 * @author Admin
 */
@WebServlet(name = "PackageSave", urlPatterns = {"/PackageSave"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          PackageCourse packageCourse=new PackageCourse();
            ...
            packageCourse.setAdminOfBranch(Branch);
            PackageDao PackageDao=new PackageDao();
             boolean b=PackageDao.saveaPackage(packageCourse);
                 
                if (b) {
                    int i = 1;
                    HttpSession session = request.getSession();
                    session.setAttribute("i", i);
                    session.setAttribute("msg", "Package Add Successfully");
