...
import com.accede.angel.sell.dao.ExchangeDao;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (Branch != null) {
            ...
            if (c) {
                ...
                String custsaleid = request.getParameter("custsaleid");

                CustomerSaleDetailsDAO b1 = new CustomerSaleDetailsDAO();
                CustomerSaleDetails c1 = b1.getCustomerSaleDetailsByID(Integer.parseInt(exchangeid));

                CustomerSaleDAO csd1 = new CustomerSaleDAO();
                CustomerSale s1=csd1.getpurchaseByID(Integer.parseInt(custsaleid));
                ...
                if (b) {
                    ...
                    ExchangeDao exd=new ExchangeDao();
                    BranchProduct bp1 = new BranchProduct();
                    bp1.setAdminid(Branch.getAdminId());
                    bp1.setProductid(c1.getProduct().getId());
                    bp1=bpdao.getBranchProduct(bp1);
                    long availabelquantity=bp1.getProductquantity();
                    availabelquantity=availabelquantity+c1.getProductquantity();
                    bp1.setProductquantity(availabelquantity);
                    bpdao.updateBranchProduct(bp1);
                    
                    
                    ex.setCustomerPurchaseProduct(s1);
                    ex.setProductquantity(c1.getProductquantity());
                    ex.setGross_amount(c1.getGross_amount());
                    ex.setPurchasePrice(c1.getPurchasePrice());
                    ex.setMrpprice(c1.getMrpprice());
                    ex.setPurchaseDate(c1.getDate());
                    ex.setExchangeDate(new Date());
                    ex.setCustomerSale(cs);
                    ex.setCustomerSaleDetails(c1);
                  ex.setProduct(c1.getProduct());
                    ex.setCustomerAccount(CustomerAccount);
                    exd.SaveProduct(ex);
